...
class QLClient(ClientApi):
    def __init__(self, client_info: Dict):
        ...
        self.delete = client_info.get("delete", False)
    ...
    def init_cron(self):
        ...
        if len(self.exclude) > 0:
            has_exclude = True
        print('repo为：' + self.repo)
        print('exclude为：' + self.exclude)
        self.cron: List[Dict] = list(
            filter(lambda x: x.get("command", "").find(self.repo) != -1
        ...
        print("获取到待清理任务总数为" + str(len(self.cron)) + "个。")
    ...
    def clean_cron(self):
        ...
        for c in self.cron:
            ...
            if 'id' in c:
                ...
                time_now = int(round(time.time() * 1000))
                if self.delete:
                    res = requests.delete(url=self.url + "open/crons?t=" + str(time_now),
                                   json=data,
                                   headers={"Authorization": f"Bearer {self.token}"}).json()
                else:
                    res = requests.put(url=self.url + "open/crons/disable?t=" + str(time_now),
                                   json=data,
                                   headers={"Authorization": f"Bearer {self.token}"}).json()
...
def get_client():
    ...
    if env_type == 5 or env_type == 6:
        check_data = get_data()
        return QLClient(check_data.get("BATCH_CLEAN_TASKS", [[]])[0])
...
try:
    print("批量清理任务开始......")
    res = get_client().run()
    msg = "批量清理任务完成！总计：" + str(res[0]) + "个， 成功：" + str(res[1]) + "个。"
